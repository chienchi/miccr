...
print(args)
classified_result_table<-args[1]
out_prefix<-args[2]
cov_table_file <-args[3]
top<-30  # only plot top30 if more than 30 unique taxas

#Rscript contig_classification_plot2.R HMPeven.ctg.tsv.lineage test HMPeven.coverage.txt

# check input
if ( !file.exists(classified_result_table) ){
	cat( " ./classification_plot.R  ctg_class.csv out_prefix readsToContigs_coverage.table\n")
	quit(save="no")
}
...
{
	# Remove extra header
	taxa_table<-taxa_table[-c(which(taxa_table$RANK=="LCA_RANK")),]

	# re factor of the column
	taxa_table$AGG_LENGTH<-factor(taxa_table$AGG_LENGTH)

	# Factor to numeric conversion
	taxa_table$AGG_LENGTH<-as.numeric(levels(taxa_table$AGG_LENGTH))[taxa_table$AGG_LENGTH]
}
...
for ( i in 1:7) {
	...
	#data.rank <- subset(taxa_table, (RANK == ranklist[i] | RANK == "unclassified") & AGG_LENGTH > 0)
  	...
  	bp_plot_table<-bp_plot_table[order(bp_plot_table$AGG_LENGTH,decreasing = TRUE),]
	colnames(bp_plot_table)[1]<-"ORGANISM"
  	
  	# get contig assignemnt by top hit 
	#data.rank.forTopHit<-aggregate(AGG_LENGTH~CONTIG , data=data.rank, FUN=max)
	#data.rank.forTopHit<-subset(data.rank, (RANK == ranklist[i] | (RANK == "unclassified" & LENGTH == ACC_COV_LEN)))
  	top_hit_table<-merge(aggregate(AGG_LENGTH ~ CONTIG, data=data.rank, FUN=max), data.rank)
 
	if (file.exists(cov_table_file)){
  		# get depth and GC info by merging table with contig id 
  		top_hit_table_with_cov<-merge(cov_0_above_table,top_hit_table,by.x="ID",by.y="CONTIG")
  	
  		# re factor of the subset
  		#top_hit_table_with_cov[ranklist[i]]<-factor(top_hit_table_with_cov[ranklist[i]])

  		#count by taxanomy assignments
  		count<-table(as.matrix(top_hit_table_with_cov[ranklist[i]]))
  		# unique taxa number
  		total_tax<-length(count)
	}else{
  		#count by taxanomy assignments
  		count<-table(as.matrix(top_hit_table[ranklist[i]]))
  		# unique taxa number
  		total_tax<-length(count)
	}
	...
	if (file.exists(cov_table_file)){
  		top_hit_table_with_cov$COLOR<-rep("darkgrey",length(top_hit_table_with_cov[ranklist[i]])) 
	}
  	...
  	for (j in 1:length(unique_org))
  	{
		...
		if (file.exists(cov_table_file)){
  			top_hit_table_with_cov$COLOR[which(top_hit_table_with_cov[ranklist[i]]==unique_org[j])]<-unique_c[j] 
		}
	 ...
	 ## color not used in bp_plot_table, the different taxa top list between count and length
  	...
  	barplot(bp_plot_table$AGG_LENGTH,names.arg=bp_plot_table$ORGANISM,col=bp_plot_table$COLOR,las=3,cex.names=0.8,ylab="Length Of Contigs (bp)")
  	if (total_tax>top){
